...
if (localStorage.getItem('isUserLoggedIn') === "false") {
    content = <button
        ...
        <Link className="text-gray-200 text-smaller hover:text-gray-50 transition-all" to="/signup">Sign Up</Link>
    </button>;
} else {
    content =
        <div className="hidden md:flex justify-center items-center gap-2">
            ...
            <Link to="myAccount">
                <div className="flex items-center justify-center gap-2">
                    <div className="p-0.5 bg-secondary bg-opacity-100 rounded-full">
                        <img className="w-7 rounded-full object-scale-down filter saturate-150"
                             src={`data:image/png;base64, ${localStorage.getItem('profileImg')}`} alt=""/>
                    </div>
                    <h6 className="hidden text-smaller lg:block">{localStorage.getItem('username')}</h6>
                </div>
            </Link>
...
export class NavBar extends Component {
    ...
    render() {
        ...
        return (
            <>
                <div
                    className="hidden flex bg-black z-20 bg-opacity-5 p-3 rounded-none fixed text-normal text-white font-poppins justify-around w-full top-5"
